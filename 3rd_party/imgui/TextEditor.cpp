...
void TextEditor::RemoveLine(int aStart, int aEnd)
{
	...
	Breakpoints btmp;
	for (auto i : mBreakpointLines)
	{
		if (i >= aStart && i <= aEnd)
			continue;
		btmp.insert(i >= aStart ? i - 1 : i);
	}
	mBreakpointLines = std::move(btmp);
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			if (mCurrentBreakpoint == lineNo + 1)
			{
				auto end = ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
				drawList->AddRectFilled(start, end, mPalette[(int)PaletteIndex::BreakpointLine]);
			}
			...
			const ImVec2 brkEnd(lineStartScreenPos.x + mTextStart, lineStartScreenPos.y);
			const float fontSize = ImGui::GetFontSize();
			const ImRect brkBounds(ImVec2(brkEnd.x - fontSize, brkEnd.y - fontSize), brkEnd);
			const ImGuiID brkId = 0xfb84c69 + lineNo;
			bool isHoveredBrk = false;
			if (ImGui::ItemAdd(brkBounds,  brkId)) {
				isHoveredBrk = ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem);
				if (ImGui::ButtonBehavior(brkBounds, brkId, nullptr, nullptr)) {
					if (mBreakpointLines.contains(lineNo)) {
						mBreakpointLines.erase(lineNo);
					} else {
						mBreakpointLines.emplace(lineNo);
					}
				}
			}

			if (isHoveredBrk || mBreakpointLines.count(lineNo) != 0)
			{
				const float fontSize = ImGui::GetFontSize() * 0.5f;
				const float radius = fontSize * 0.9f;
				ImVec2 cirCenter(lineStartScreenPos.x + mTextStart - fontSize, lineStartScreenPos.y - fontSize);

				drawList->AddCircleFilled(cirCenter, radius, mPalette[(int)PaletteIndex::BreakPoint] ^ (0x70000000 * isHoveredBrk));
				drawList->AddCircle(cirCenter, radius, mPalette[(int)PaletteIndex::Default] ^ (0x70000000 * isHoveredBrk), 0, 1.5f);
			}
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			...
			0x40f08000, // BreakpointLine
			0xff707000, // Line number
			0x40000000, // Current line fill
			0x40808080, // Current line fill (inactive)
			0x40a0a0a0, // Current line edge
			0xff0020ff, // Breakpoint
		} };
	return p;
}
...
const TextEditor::Palette & TextEditor::GetLightPalette()
{
	const static Palette p = { {
			...
			0x80f08000, // BreakpointLine
			0xff505000, // Line number
			0x40000000, // Current line fill
			0x40808080, // Current line fill (inactive)
			0x40000000, // Current line edge
			0xff0010ff, // Breakpoint
		} };
	return p;
}
			...
			else
			{
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					if (!line[first - bufferBegin].mPreprocessor)
					{
						...
						else if (mLanguageDefinition.mIdentifiers.count(id) != 0 || *token_end == '(')
